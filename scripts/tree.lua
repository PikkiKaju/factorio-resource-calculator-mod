...
local function add_tree_node(parent, node_info, layer, column, is_last, compact_mode)
    ...
    if compact_mode then
        node_flow.style.padding = 0
        node_flow.style.vertical_spacing = 0
    else
    ...
    if compact_mode then
        numbers_flow.style.padding = 0
        numbers_flow.style.vertical_spacing = 0
    else
        numbers_flow.style.padding = 4
        numbers_flow.style.vertical_spacing = 2
    end

    -- Add the production rate label
    if node_info.item_amount_per_second then
        local rate_label = compact_mode and { "gui-tree.production-rate-label-short" } or
        { "gui-tree.production-rate-label-long" }
        local rate_value = 0
    ...
    if node_info.machines_amount then
        local machines_label = compact_mode and { "gui-tree.machines-amount-label-short" } or
        { "gui-tree.machines-amount-label-long" }
    ...
    -- Add the item name label if not in compact mode
...
local function add_tree_layer(parent, layer, column, node_spacing, compact_mode)
    ...
    main_item.style.horizontally_stretchable = false
    main_item.style.vertically_stretchable = true
    ...
    ingredients.style.horizontally_stretchable = false
    ingredients.style.vertically_stretchable = true
    if compact_mode then
        ingredients.style.padding = 0
        ingredients.style.vertical_spacing = 0
    else
        ingredients.style.padding = 2
        ingredients.style.vertical_spacing = 2
    end
...
local function add_graphicap_recipe_tree_to_gui(parent, recipe_table, layer, column, compact_mode, raw_ingredients_mode)
    ...
    local layer_flows = add_tree_layer(parent, layer, column, 10, compact_mode)
    ...
    if recipe_table.ingredients and #recipe_table.ingredients > 0 then
        -- Flexible spacer above ingredient nodes to help vertical centering
        local top_spacer = layer_flows.ingredients.add { type = "empty-widget" }
        ...
        for _, ingredient in ipairs(recipe_table.ingredients) do
            add_graphicap_recipe_tree_to_gui(layer_flows.ingredients, ingredient, layer + 1, child_column, compact_mode,
                raw_ingredients_mode)
            child_column = child_column + 1
        end

        -- Flexible spacer below ingredient nodes to complete centering
        local bottom_spacer = layer_flows.ingredients.add { type = "empty-widget" }
        bottom_spacer.style.vertically_stretchable = true
    end
end
